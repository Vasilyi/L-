...
namespace Marksman.Champions
{
    internal class Lucian : Champion
    {
        ...
        public Lucian()
        {
            ...
            Utils.Orbwalking.AfterAttack += Orbwalking_AfterAttack;
        }
        ...
        {
            if ((!Utils.Orbwalking.CanAttack(-150) && !Utils.Orbwalking.CanAttack() && Utils.Orbwalking.CanMove(0) && !ObjectManager.Player.Spellbook.IsCastingSpell) || GetAATarget() == null)
            {
                return true;
            }
        ...
        public override void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (Q.IsReady() && GetValue<KeyBind>("UseQExtendedTH").Active && ToggleActive)
            {
                ...
                {
                    if (ObjectManager.Player.Distance(t) > Q.Range && ShouldCast())
            ...
            if (useE != 0 && E.IsReady())
            {
                if (t.IsValidTarget(Q.Range) && ShouldCast())
                {
        ...
        public override void ExecuteLaneClear()
        {
            ...
            if (laneWValue != 0 && E.IsReady() && ShouldCast())
            {
        ...
        public static Obj_AI_Hero GetAATarget()
        {
            ...
            if (target != null && Utils.Orbwalking.InAutoAttackRange(target))
            {
                return target;
            }
            return null;
        ...
        public override void PermaActive()
        {
            ...
            if (enemy != null)
            {
                ...
                {
                    if (enemy.IsValidTarget() && Utils.Orbwalking.InAutoAttackRange(enemy))
                    {
                        if (!Q.IsReady())
                        {
                            if (W.IsReady())
