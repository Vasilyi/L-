...
namespace Marksman.Utils
{
    ...
    public static class Orbwalking
    {
        ...
        private static readonly string[] AttackResets =
            {
                ...
                "jaycehypercharge", "leonashieldofdaybreak", "luciane", "monkeykingdoubleattack", "mordekaisermaceofspades",
        ...
        public static bool CanAttack(float tempdelay)
        {
            if (Player.ChampionName == "Graves" && Attack)
            {
                var attackDelay = 1.0740296828d * 1000 * Player.AttackDelay - 716.2381256175d;
                if (LeagueSharp.Common.Utils.GameTimeTickCount + Game.Ping / 2 + 25 >= LastAATick + tempdelay + attackDelay && Player.HasBuff("GravesBasicAttackAmmo1"))
                {
                    return true;
                }
        ...
        public static void MoveTo(Vector3 position, float holdAreaRadius = 0, bool overrideTimer = false, bool useFixedDistance = true, bool randomizeMinDistance = true)
        {
            ...
            if (LeagueSharp.Common.Utils.GameTimeTickCount - LastMoveCommandT < (70 + Math.Min(60, Game.Ping)) && !overrideTimer && angle < 60)
            {
                return;
            }
        ...
        public class Orbwalker
        {
            ...
            /// </summary>
            private static Vector3 _orbwalkingPoint;
            ...
            public Orbwalker(Menu attachToMenu)
            {
                ...
                misc.AddItem(
                    new MenuItem("HoldPosRadius", "Hold Position Radius").SetShared().SetValue(new Slider(85, 0, 250)));
                ...
                config.AddItem(new MenuItem("FarmDelay", "Farm delay").SetShared().SetValue(new Slider(30, 0, 200)));
            ...
            public static float AttackSpeedDelay
            {
                get
                {
                    if (!config.Item("AttackSpeedLimiter.Enabled").GetValue<bool>())
                    {
                        return ObjectManager.Player.AttackDelay;
                    }

                    var speed = (float)config.Item("AttackSpeedLimiter.MaxAttackSpeed").GetValue<Slider>().Value;

                    switch (config.Item("AttackSpeedLimiter.LimitWhen").GetValue<StringList>().SelectedIndex)
                    {
                        case 0:
                            {
                    ...
                    ObjectManager.Get<Obj_AI_Minion>()
                        .Any(
                            ...
                            && HealthPrediction.LaneClearHealthPrediction(
                                minion, (int)((AttackSpeedDelay * 1000) * LaneClearWaitTimeMod), FarmDelay) <= player.GetAutoAttackDamage(minion));
            }
                    ...
                    if (!ShouldWait())
                    {
                        if (prevMinion.IsValidTarget() && InAutoAttackRange(prevMinion))
                        {
                            var predHealth = HealthPrediction.LaneClearHealthPrediction(
                                prevMinion,
                                (int)((AttackSpeedDelay * 1000) * LaneClearWaitTimeMod),
                                FarmDelay);
            ...
            private void Game_OnUpdate(EventArgs args)
            {
                ...
                //Game.PrintChat("Attack Speed Delay: " + AttackSpeedDelay.ToString() + " | Player Attack Delay:" + Player.AttackDelay);
                try
                {
                    ...
                    Orbwalk(target, (_orbwalkingPoint.To2D().IsValid()) ? _orbwalkingPoint : Game.CursorPos, config.Item("ExtraWindup").GetValue<Slider>().Value, 
            ...
            private void Drawing_OnDraw(EventArgs args)
            {
                ...
                {
                    Render.Circle.DrawCircle(player.Position,GetRealAutoAttackRange(null) + 65,config.Item("AACircle").GetValue<Circle>().Color,1);
                }
